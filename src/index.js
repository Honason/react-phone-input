...
class ReactPhoneInput extends React.Component {
  constructor(props) {
    ...
    let onlyCountries = excludeCountries(
      getOnlyCountries(props.onlyCountries),
      props.excludeCountries
    );
    let selectedCountryGuess = find(onlyCountries, {
      iso2: this.props.defaultCountry
    });
  ...
  handleFlagItemClick(country) {
    ...
    if (currentSelectedCountry.iso2 !== nextSelectedCountry.iso2) {
      ...
      let newNumber = this.state.formattedNumber.replace(
      ...
      >
        ...
        >
          <div
            ...
            title={`${this.state.selectedCountry.name}: + ${this.state
...
ReactPhoneInput.prototype.guessSelectedCountry = memoize(function(
  ...
  var secondBestGuess =
    find(allCountries, { iso2: defaultCountry }) || onlyCountries[0];
  if (trim(inputNumber) !== "") {
